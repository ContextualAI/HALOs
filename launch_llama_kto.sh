#!/bin/bash
#SBATCH --job-name=llama-fixed-KL
#SBATCH --nodes=1
#SBATCH --mem=100G
...
export CKPT=/scratch/gpfs/ke7953/models/llama3-8B-kto-${BETA}-${LR}-${SCH}-${DECAY}/FINAL
...
accelerate launch \
    ...
    launch.py loss=kto model=llama datasets=[ultrabin] exp_name=llama3-8B-kto-${BETA}-${LR}-${SCH}-${DECAY} \
    ++cache_dir=/scratch/gpfs/ke7953/models \
    ++model.name_or_path=\$MODEL_PATH \
    ++lr=${LR} \
    ++loss.beta=${BETA} \
    ++model.batch_size=8 ++model.gradient_accumulation_steps=4 ++model.eval_batch_size=8 \
    ++loss.schulman_KL=${SCH} ++loss.KL_decay=${DECAY}
...
python -m eval.sample_for_alpacaeval \$CKPT --gpu_count 2 --output_file outputs/llama3-8b-kto-${BETA}-${LR}-${SCH}-${DECAY}.json
"