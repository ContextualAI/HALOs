...
export CACHE_DIR=/scratch/gpfs/ke7953/models/llama-instruct-dpo-online-${BETA}-${LR}-${L}-${U}-${NORM}-${ITERS}-humanline
...
if [ \$CUMULATIVE_PROMPTS -eq 0 ]; then
    ...
    CURRENT_CKPT=\${CACHE_DIR}/llama3-8B-instruct-dpo-\${CUMULATIVE_PROMPTS}/FINAL
fi
