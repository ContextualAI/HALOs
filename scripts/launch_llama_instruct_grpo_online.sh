#!/bin/bash
#SBATCH --job-name=llama-instruct-online-grpo
#SBATCH --nodes=1
#SBATCH --mem=60G
...
export CACHE_DIR=/scratch/gpfs/ke7953/models/llama-instruct-grpo-online-${BETA}-${EPS}-${LR}-${NORM}
...
if [ \$CUMULATIVE_PROMPTS -eq 0 ]; then
    ...
    CURRENT_CKPT=\${CACHE_DIR}/llama3-8B-instruct-grpo-\${CUMULATIVE_PROMPTS}/FINAL
fi
...
while [ \$CUMULATIVE_PROMPTS -lt ${TOTAL_PROMPTS} ]; do
    ...
    if [ \$PYTHON_EXIT_CODE -eq 0 ]; then
        ...
        accelerate launch \
            ...
            --main_process_ip \$MASTER_ADDR \
            --main_process_port \$MASTER_PORT \
            launch.py loss=grpo ++loss.beta=${BETA} ++loss.epsilon=${EPS} model=llama exp_name=\$EXP_NAME \
