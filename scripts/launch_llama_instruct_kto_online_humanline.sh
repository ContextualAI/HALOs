...
export CACHE_DIR=/scratch/gpfs/ke7953/models/llama-instruct-kto-online-${BETA}-${LR}-${L}-${U}-${NORM}-humanline
...
if [ \$CUMULATIVE_PROMPTS -eq 0 ]; then
    ...
    CURRENT_CKPT=\${CACHE_DIR}/llama3-8B-instruct-kto-\${CKPT_PROMPTS}/FINAL
fi
...
while [ \$CUMULATIVE_PROMPTS -lt ${TOTAL_PROMPTS} ]; do
    ...
    if [ \$PYTHON_EXIT_CODE -eq 0 ]; then
        ...
        accelerate launch \
            ...
            --main_process_ip \$MASTER_ADDR \
            --main_process_port \$MASTER_PORT \
            launch.py loss=kto ++loss.beta=${BETA} ++loss.desirable_weight=1.1 model=llama exp_name=\$EXP_NAME \
