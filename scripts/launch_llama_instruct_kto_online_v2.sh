...
#SBATCH --output=logs/llama-okto-v2-%j.out
#SBATCH --error=logs/llama-okto-v2-%j.err
...
export CACHE_DIR=/scratch/gpfs/sl2998/models/llama-instruct-kto-online-prompt-v2
...
while [ \$CUMULATIVE_PROMPTS -lt ${TOTAL_PROMPTS} ]; do
    ...
    if [ \$PYTHON_EXIT_CODE -eq 0 ]; then
        ...
        accelerate launch \
            ...
            ++model.batch_size=8 ++model.gradient_accumulation_steps=4 ++model.eval_batch_size=8 ++model.max_grad_norm=0.1 \
