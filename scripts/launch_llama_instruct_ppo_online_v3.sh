#!/bin/bash
#SBATCH --job-name=llama-oppo-binary-v3
#SBATCH --nodes=1
#SBATCH --mem=100G
...
export CACHE_DIR=/scratch/gpfs/sl2998/models/llama-3-8b-instruct-online-ppo-kl-${KL_COEF}-lr-${LR}-clip-${CLIP_RANGE}-binary-v3
...
for r in \$(seq ${NUM_ROUNDS} -1 1); do
    LAST_EXP_NAME=llama-3-8b-instruct-online-ppo-kl-${KL_COEF}-lr-${LR}-clip-${CLIP_RANGE}-binary-v3-R\${r}
...
while [ \$ROUND -le ${NUM_ROUNDS} ]; do
    ...
    if [[ \$NUM_SAMPLES -gt 0 ]]; then
        EXP_NAME=llama-3-8b-instruct-online-ppo-kl-${KL_COEF}-lr-${LR}-clip-${CLIP_RANGE}-binary-v3-R\${ROUND}
        ...
        ssh della-gpu \"source ~/.bashrc && \
            ...
            accelerate launch -m train.label_v3 \$SAMPLES_FILE \$DATA_FILE \
